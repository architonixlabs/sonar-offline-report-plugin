...
Version `2.0.1` is a **production-hardening release candidate** targeting SonarQube Community Build `26.6.0.123539`. Report Model v3, portfolio reporting, persona evidence views, artifact-level completeness, cancellation-safe collection and cross-format reconciliation are implemented and automated locally. The exact published v2.0.1 JAR has passed a Linux Docker atomic upgrade, startup/log review, route/static byte-integrity smoke test and rollback-backup verification; the operating procedure was also rehearsed end to end with the model-equivalent lab candidate. Promotion still requires authenticated authorization, HTTPS, target-server load, desktop Office, accessibility and policy approval of the clean release evidence in [Enterprise readiness](docs/ENTERPRISE-READINESS.md).
...
The v2.0.1 candidate is verified by the automated Model v3, persona, cross-format, browser-renderer and packaging suites. The exact clean-release JAR, project/global routes and static bundles have passed target-server startup and byte-integrity checks; authenticated current-user inventory, trend collection, portfolio limits and negative permissions still require live qualification. Desktop Word/Excel/LibreOffice checks also remain open. See [Compatibility and acceptance](docs/COMPATIBILITY.md), the [exact v2.0.1 deployment record](docs/DEPLOYMENT-VALIDATION-V2.0.1-2026-08-22.md), and the [historical rollback rehearsal](docs/DEPLOYMENT-VALIDATION-2026-08-22.md).
