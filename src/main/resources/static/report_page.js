/* SonarQube Offline Report Plugin 1.2.0 - generated; edit src/main/js and run npm run build. */
window.OfflineReportBuild = Object.freeze({ pluginVersion: "1.2.0" });
(function (global) {
  "use strict";
...
try{init();}catch(error){renderFailure(error);}
  ...
  const REPORT_RUNTIME_SHA256 = "W5rbwO4TfRIbqZcF2afKYJhgC4EMStnLktbVsfDIhtg=";
...
<title>${app.escapeHtml(template.title)}</title><style>${reportStyles(template.accentColor)}${professionalStyles()}.condition-required,.condition-variance{display:block;color:var(--muted);font-size:.85rem}.condition-variance{font-weight:700;margin-top:.2rem}.gate-note{margin:.65rem 0 0;color:var(--muted);font-size:.85rem}.print-manifest{margin:1rem 0;padding:1rem;border:2px solid var(--ink);background:#fff}.print-manifest dl{display:grid;grid-template-columns:max-content 1fr;gap:.25rem 1rem;margin:.5rem 0 0}.print-manifest dt{font-weight:700}.print-manifest dd{margin:0}</style></head>
